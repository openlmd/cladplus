...
class QtPlot():
    ...
    def plot(self):
        ...
        x = plt.subplot(2, 1, 2)
        x.set_ylim(self.min_power, self.max_power)
        x.plot(self.control[:, 0], self.control[:, 1], color="blue",
               linewidth=1.0, linestyle="-")
        y = plt.subplot(2, 1, 1)

        y.set_ylim(self.min_meas, self.max_meas)
        y.plot(self.geometry[:, 0], self.geometry[:, 1], color="red",
               linewidth=1.0, linestyle="-")
        plt.show()
